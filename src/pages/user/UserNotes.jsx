...
const UserNotes = () => {
  ...
  const uid = user?.uid;
  const myNotes = useMemo(() => notes.filter((n) => n.userId === uid || n.authorId === uid), [notes, uid]);
  ...
  const categories = useMemo(() => {
    const cats = new Set(myNotes.map((n) => n.category).filter(Boolean));
    return [...cats];
  }, [myNotes]);
  ...
  return (
    <div className="dashboard-content">
      <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem', marginBottom: '1.5rem' }}>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
          <div className="section-header">
            <h1>My Notes</h1>
            <p>{myNotes.length} notes</p>
          </div>
          <button className="btn btn-primary btn-sm" onClick={openCreate}>New Note</button>
        </div>
      ...
      <Modal isOpen={modalOpen} onClose={() => { setModalOpen(false); setEditingNote(null); }} title={editingNote ? 'Edit Note' : 'New Note'}>
