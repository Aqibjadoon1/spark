...
const UserTasks = () => {
  ...
  const displayed = useMemo(() => {
    ...
    if (sortBy === 'dueDate') list.sort((a, b) => (a.dueDate || '').toString().localeCompare(b.dueDate?.toString() || ''));
    if (sortBy === 'priority') {
    ...
    return list;
  }, [myTasks, search, statusFilter, priorityFilter, sortBy]);
  ...
  const handleSubmit = async (e) => {
    ...
    setSubmitting(true);
    try {
      const payload = { ...form, assignedTo: uid, createdAt: new Date().toISOString() };
      if (editingTask) {
  ...
  const advanceStatus = async (task) => {
    const next = { todo: 'in-progress', 'in-progress': 'done', done: 'done' };
    const newStatus = next[task.status] || 'todo';
    if (newStatus === task.status) return;
    try {
      await updateTask(task.id, { ...task, status: newStatus });
      dispatch(showToast(`Status updated to ${newStatus}`, 'success'));
    } catch (err) {
      dispatch(showToast(err.message, 'error'));
    }
  };
    ...
    <div className="dashboard-content">
      <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem', marginBottom: '1.5rem' }}>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
          <div className="section-header">
            <h1>My Tasks</h1>
            <p>{myTasks.length} tasks assigned to you</p>
          </div>
      ...
      {loading ? (
        ...
        <EmptyState
          ...
          actionLabel={search ? undefined : 'New Task'}
            ...
            <table style={{ width: '100%', fontSize: '0.875rem', borderCollapse: 'collapse' }}>
              <thead>
                <tr style={{ background: '#f5f5f5' }}>
                  ...
                  <th style={{ padding: '1rem 1.5rem', textAlign: 'left', fontSize: '0.75rem', fontWeight: '600', color: '#888', textTransform: 'uppercase', letterSpacing: '0.05em' }}>Category</th>
                ...
                {paginated.map((task) => (
                  <tr key={task.id} style={{ borderTop: '1px solid #eee' }}>
                    ...
                    <td style={{ padding: '1rem 1.5rem' }}>
                      <button onClick={() => advanceStatus(task)} style={{ cursor: 'pointer', background: 'none', border: 'none', padding: 0 }}>
                    ...
                    <td style={{ padding: '1rem 1.5rem', color: '#888' }}>{task.category || '-'}</td>
