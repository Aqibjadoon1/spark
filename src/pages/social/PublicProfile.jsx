    ...
    <div className="animate-in">
      <div style={{ maxWidth: 1060, margin: '0 auto' }}>
        ...
        <div className="profile-ig-header">
          <div className="profile-ig-row">
            <div className="profile-ig-avatar-col">
              <img
                className="profile-ig-avatar"
                src={profileUser.photoURL}
                alt={profileUser.displayName}
              />
              {profileUser.online && <span className="profile-ig-online" />}
            </div>

            <div className="profile-ig-info-col">
              <div className="profile-ig-stats-row">
                <span className="profile-ig-stat-item"><strong>{userPosts.length}</strong> posts</span>
                <span className="profile-ig-stat-item"><strong>{friendCount}</strong> friends</span>
                <span className="profile-ig-stat-item"><strong>{totalReactions}</strong> reactions</span>
              </div>

              <h2 className="profile-ig-name">{profileUser.displayName || 'Unknown User'}</h2>

              {profileUser.bio && <p className="profile-ig-bio">{profileUser.bio}</p>}

              {Object.values(socialLinks).some(Boolean) && (
                <div className="profile-ig-social">
                  {socialLinks.website && (
                    <a href={socialLinks.website} target="_blank" rel="noopener noreferrer" className="profile-ig-link">
                      {socialLinks.website.replace(/^https?:\/\//, '')}
                    </a>
                  )}
                  {socialLinks.twitter && (
                    <a href={socialLinks.twitter} target="_blank" rel="noopener noreferrer" className="profile-ig-social-icon" aria-label="Twitter">
                      <svg width="16" height="16" viewBox="0 0 24 24" fill="currentColor"><path d="M18.244 2.25h3.308l-7.227 8.26 8.502 11.24H16.17l-5.214-6.817L4.99 21.75H1.68l7.73-8.835L1.254 2.25H8.08l4.713 6.231zm-1.161 17.52h1.833L7.084 4.126H5.117z" /></svg>
                    </a>
                  )}
                  {socialLinks.github && (
                    <a href={socialLinks.github} target="_blank" rel="noopener noreferrer" className="profile-ig-social-icon" aria-label="GitHub">
                  ...
                  {socialLinks.linkedin && (
                    <a href={socialLinks.linkedin} target="_blank" rel="noopener noreferrer" className="profile-ig-social-icon" aria-label="LinkedIn">
              ...
              <div className="profile-ig-actions">
                {isOwnProfile ? (
                  <button className="profile-ig-edit-btn" onClick={() => navigate('/profile/edit')}>Edit profile</button>
                ) : (
                  <>
                    {isFriend ? (
                      <button className="profile-ig-friend-btn" disabled>Friends</button>
                    ) : requestSent ? (
                      <button className="profile-ig-requested-btn" disabled>Request sent</button>
                    ) : (
                      <button className="profile-ig-follow-btn" onClick={handleSendRequest}>Follow</button>
                    )}
                    <button className="profile-ig-message-btn" onClick={handleMessage}>Message</button>
                  </>
                )}
              </div>
            </div>
          ...
          {!canViewFull && (
            <div className="profile-ig-private">
              ...
              {!isOwnProfile && !requestSent && !isFriend && (
                <button className="profile-ig-follow-btn" onClick={handleSendRequest}>Follow</button>
              )}
            </div>
          )}
